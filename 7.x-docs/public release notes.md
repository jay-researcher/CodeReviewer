...
状态：已于 2026-07-24 部署到 `192.168.3.78:8765`，服务与数据健康检查通过。

- 新报告会携带明确的 Sprint/Cycle 身份；当前交付报告不会再被静默归入 Legacy。
- Issue Review History 会保留本周报告与处理进度，并按真实 Sprint/Cycle 展示。
- Scope 扩大时保留既有证据，但必须补齐新增 Scope 后才能再次 Pass。
- 生产上的错误 Legacy 分类已清理；本周报告和处理记录均已迁入真实 Sprint/Cycle 或明确的 Ad hoc 历史 Cycle。

## 7.2.16+b202607241318 — 2026-07-24

状态：已于 2026-07-24 部署到 `192.168.3.78:8765`，服务与数据健康检查通过。

- WVAdmin、MOMD、AOP/LCA 与 Web 公共仓库使用明确的 Repository Responsible 边界；Jira 交付负责人和 Web Reviewer 权限保持独立。
- 扫描开始后自动折叠输入区时不再推动整个页面，顶部导航和 Run Review 上下文保持可见。
- 用户拖动 Progress 垂直滚动条期间不会被自动跟随抢走位置；释放后等待 60 秒才恢复。

## 7.2.16+b202607231300 — 2026-07-23

状态：已于 2026-07-23 部署到 `192.168.3.78:8765`，健康检查与空数据验收通过。

- 无可审核 MR 的当前 Cycle 会显示 `No Review Required`，不会误计为 Passed、Remaining 或待生成报告。
- Issue 列表、详情与 Overview 使用一致的 Cycle Scope，避免旧 Cycle 状态影响当前工作流。
- 本次发布将重置历史报告与关联 Workflow 审核数据；配置和用户保持不变，后续 Scan 从当前 Jira/MR 重新建立交付范围。
- 既有报告与 Workflow 审核记录已完成备份后清空；当前报告数为 0，后续需要重新执行 Sprint Scan 和 Review。

## 7.2.16 — 2026-07-23

状态：已于 2026-07-23 部署到 `192.168.3.78:8765`，生产健康检查、配置策略、账户、Workflow 历史数据及回滚入口验证通过。

以下后续验收修订已包含在 `7.2.16+b202607231300`：
