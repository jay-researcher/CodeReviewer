...
Problems 中问题详情默认显示最多两行摘要，可展开全文。`Architecture / No specific file` 表示 Finding 不绑定单一文件；`New in this run`、`Still present since Run N`、`Resolved after re-scan` 表示跨 Run lineage，不是错误信息。
