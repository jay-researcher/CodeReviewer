...
- Jira Issue 必须处于允许审核的状态；7.0.2 默认只有 `Development Done`。状态不符合时，系统会在 MR 发现前跳过 Issue。
...
`gpt-5.6-sol` 使用 High reasoning 审核较大 MR 时，模型阶段可能持续数分钟。7.0.2 会正确识别路径以 `/dps`、`/dps9` 或 `/dps11` 结尾的根项目；这类项目发生 300 秒硬超时后只执行一次 Codex 尝试并直接失败，不再进入 CC Switch，也不再重复相同的长耗时请求。普通项目仍按配置的 Provider 回退策略执行。
