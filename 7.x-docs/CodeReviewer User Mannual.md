...
- `benyq.feng → kelvinh.wu`：该 Developer 使用 `kelvinh.wu` 范围；
...
生产部署前必须检查旧版本遗留的 `initial_credentials_*.txt`：先通过 User Management 为文件涉及的账户统一轮换密码，确认交付后再由获授权人员安全移除文件。当前 7.2.0 用户管理按单 Web 实例运行；多进程/多副本部署前，应将用户变更、安全审计和幂等记录迁移到同一个数据库事务。
...
#### 5.1.2 同一 Issue 的前后端问题清单

当同一个 Jira Issue 同时包含前端和后端 MR 时，CodeReviewer 根据 `config.yml` 中 repository 所属项目的 `type` 分类：

- `type: frontend`：生成前端问题清单；
- `type: backend`：生成后端问题清单。

类型配置可以位于 repository 条目本身，也可以配置在其上层项目节点并由下级 repository 继承。系统先执行同一 Issue 的端到端审核，再按 `responsible + type` 过滤 Changed Files 和 Findings，分别保存报告。即使前后端负责人相同，也不会合并为一份问题清单。
...
报告 Basic Information 中的 `Project Type` 可用于确认分类结果。未配置 `type` 的 repository 不会被猜测为前端或后端，应先补齐配置。
...
2. 在线查看 frontend/backend 报告、问题状态和 deferred Company Config/SCR 数量。
...
同一个 Jira Issue 再次回到 Backlog 并加入新 Sprint 时，系统保留旧 Complete Sprint，同时为新 Sprint 建立新的 Review Cycle。Issue History 仍是一条主记录，下面按 `Sprint → Cycle → Run Group → frontend/backend Run` 追溯。
...
Problems 在标题下分别显示“问题”和“建议”，每项最多两行；点击 `更多` 展示两项完整内容，点击 `收起` 恢复摘要。`Architecture / No specific file` 表示 Finding 不绑定单一文件；`New in this run`、`Still present since Run N`、`Resolved after re-scan` 表示跨 Run lineage，不是错误信息。
...
2. Sprint Overview 保持打开，系统以该卡片 Jira Key 启动现有单 Issue Review 流程。
3. 如果检测到仍然新鲜的报告，系统继续执行既有的复用/重新扫描确认。
4. 卡片重新扫描并切换到 `Generating`；新 Job 的详细状态同时保留在主页面 Progress 区域。
...
- `kelvinh.wu` 作为 Auditor/Responsible，`benyq.feng` 归入其负责范围；`luckxh.chen` 负责 DPS Config 与 MO Client Config。
