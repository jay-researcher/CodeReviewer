...
由于 GitHub token 缺少 `Contents: write`，本次无法从 GitHub clone。部署使用 `git archive` 生成同一提交的制品，经 SSH/SFTP 传输后解压；运行目录中的 `.deployment-commit` 固定记录上述提交 SHA。修正 token 并完成首次推送后，后续更新应改用 GitHub 仓库作为发布源。
...
- GitHub 可写 token，因此尚不能执行标准的 Git clone/pull 更新流程。
...
完成首次 GitHub 推送后，建议把现有制品目录保留为回滚副本，再部署固定 SHA：
