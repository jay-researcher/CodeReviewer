...
This is an optional Atlaskit integration, not an embedded Jira editor and not a
Forge `AdfRenderer`. Jira itself is not framed or proxied into CodeReviewer.
The interoperability contract is the stored ADF document and its validation
against Jira's documented node structure:
...
- The page exposes the active engine as `Atlaskit enhanced`, `Built-in ADF
  editor`, or `Validated ADF preview`; it never labels the fallback as native
  Atlaskit.
